...
// 1. UPDATED QUERY: Fetch Stats AND Reports in one go
const query = `{
  "stats": *[_type == "stats"][0]{
    treesPlanted,
    survivalRate,
  ...
  "reports": *[_type == "report"] | order(_createdAt desc) {
    _id,
    title,
...
function formatBytes(bytes) {
    ...
    return `${mb.toFixed(1)} MB`; // Rounds to 1 decimal place (e.g., 4.2 MB)
}
...
export default async function ImpactPage() {
  ...
  const lastUpdated = new Date(stats._updatedAt).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
  });
      ...
      <section className="bg-moss text-white pt-24 pb-32 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
            ...
            <div className="mt-12 text-xs text-olive uppercase tracking-widest">
                Last Data Update: <span className="text-white font-bold">{lastUpdated}</span>
            </div>
        </div>
      </section>
      ...
      <section className="relative z-20 -mt-20 max-w-7xl mx-auto px-6 mb-24">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-8 rounded-sm shadow-xl border-t-4 border-cypress">
                ...
                <div className="font-serif text-5xl font-bold text-moss mb-2">
                    {stats.treesPlanted ? stats.treesPlanted.toLocaleString() : '0'}
                </div>
                <div className="text-sm text-cypress font-medium flex items-center">
                    <span className="bg-cypress/10 px-2 py-1 rounded text-xs mr-2">Live</span> Field Verified
                </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-sm shadow-xl border-t-4 border-olive">
                ...
                <div className="text-sm text-olive font-medium flex items-center">
                    <span className="bg-olive/10 px-2 py-1 rounded text-xs mr-2">Monitored</span> Target: 80%
                </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-sm shadow-xl border-t-4 border-moss">
                ...
                <div className="font-serif text-5xl font-bold text-moss mb-2">
                    {stats.carbonSequestered ? stats.carbonSequestered.toLocaleString() : '0'}
                </div>
                <div className="text-sm text-moss font-medium flex items-center">
                    <span className="bg-moss/10 px-2 py-1 rounded text-xs mr-2">Verified</span> Gold Standard
                </div>
            </div>
                  ...
                  {reports.length > 0 ? (
                      reports.map((report) => (
                        <a 
                          ...
                          className="flex items-center p-6 bg-white border border-olive/10 hover:border-moss transition-colors group shadow-sm"
                        >
                            ...
                            <div>
                                <h4 className="font-bold text-moss group-hover:text-cypress">{report.title}</h4>
                                <span className="text-xs text-cedar uppercase tracking-widest">
                                    {report.extension} • {formatBytes(report.size)}
                                </span>
                            </div>
                        </a>
                      ))
                  ) : (
                      <p className="col-span-3 text-center text-gray-500">No reports uploaded yet.</p>
                  )}
